document.addEventListener("DOMContentLoaded", () => {
  ...
  function crearRamo(ramo, parentDiv) {
    ...
    div.addEventListener("click", () => {
      ...
      // Desbloquear ramos si el requisito es aprobado
      if (ramo.reqs) {
        checkAndUnlockRamos();
      }
    });
  ...
  function checkAndUnlockRamos() {
    // Revisa todos los ramos y desbloquea aquellos cuyo requisito esté aprobado
    const allRamos = document.querySelectorAll(".ramo");
    allRamos.forEach(ramo => {
      const ramoId = ramo.id;
      const ramoData = getRamoDataById(ramoId);
      if (ramoData && ramoData.reqs) {
        const desbloqueado = ramoData.reqs.every(reqId => estadoRamos[reqId]);
        if (desbloqueado && ramo.classList.contains("locked")) {
  ...
  function getRamoDataById(id) {
    const allRamos = [...mallaLicenciatura.flatMap(a => a.semestres.flatMap(s => s.ramos)), ...mallaProfesional.flatMap(a => a.semestres.flatMap(s => s.bimestres ? s.bimestres.flatMap(b => b.ramos) : s.ramos))];
    return allRamos.find(ramo => ramo.id === id);
  }
